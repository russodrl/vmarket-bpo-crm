  ...
  return <div className="flex min-h-[calc(100vh-7.5rem)] flex-col md:h-full md:min-h-0">
    <div className="border-b border-slate-200 bg-white">
      <div className="flex flex-wrap items-center gap-2 px-3 py-2 md:h-12 md:flex-nowrap md:px-4 md:py-0">
        ...
        <button className="h-11 rounded-l border border-[#087d3e] bg-[#238847] px-3 text-sm font-semibold text-white shadow-sm hover:bg-[#1f7a40] md:h-auto md:py-1.5">+ Deal</button>
        <button className="-ml-2 grid h-11 place-items-center rounded-r border border-[#087d3e] bg-[#1f7a40] px-3 text-sm font-semibold text-white md:h-auto md:py-1.5"><ChevronDown size={14}/></button>
    ...
    {pipelineView === 'kanban' ? <div className="grid min-h-0 flex-1 grid-cols-1 overflow-visible md:overflow-hidden xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="min-w-0 overflow-x-auto overflow-y-visible p-3 md:overflow-y-hidden md:p-4">
        <div className="flex min-h-[420px] gap-3 md:h-full md:min-w-max">
          {stages.map((stage) => {
            ...
            return <div key={stage.id} onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDrop(e, stage.id)} className="flex max-h-[65vh] min-w-[78vw] flex-col bg-[#f0f2f4] ring-1 ring-slate-200 sm:min-w-[320px] md:h-full md:w-[142px] md:min-w-0 xl:w-[146px]">
...
function MobileDealDetail({ selected, openDealPage, selectedStage, activePipeline, ageDays, stages, selectedStageIndex, moveDeal, selectedActivities, completeActivity }: {
  ...
  if (!selected) return <div className="mx-3 mb-4 rounded-xl bg-white p-4 text-sm text-slate-500 ring-1 ring-slate-200 xl:hidden">Toque em um negócio para ver os detalhes.</div>
  return <section className="mx-3 mb-4 rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 xl:hidden">
    ...
    <div className="overflow-x-auto border-b border-slate-100 p-3">
      <div className="flex min-w-max gap-1">
        {stages.map((stage, i) => <button key={stage.id} onClick={() => void moveDeal(stage.id, selected.id)} className={cn('min-h-11 min-w-[90px] rounded px-2 text-[11px] font-semibold', i <= selectedStageIndex ? 'bg-[#27864d] text-white' : 'bg-slate-100 text-slate-500')}>{i === selectedStageIndex ? `${ageDays} dias` : stage.name}</button>)}
      </div>
    </div>
    ...
    <div className="border-t border-slate-100 p-4">
      <h3 className="mb-3 font-bold">Atividades</h3>
      <div className="space-y-2">
        {selectedActivities.length ? selectedActivities.map((a) => <div key={a.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
          <div className="flex items-start gap-3">
            <button onClick={() => void completeActivity(a.id)} className="mt-1 h-6 w-6 shrink-0 rounded-full border-2 border-slate-300 bg-white hover:border-emerald-500" aria-label="Marcar atividade como feita" />
            <div className="min-w-0">
              <b className="block leading-snug">{a.title}</b>
              <p className="mt-1 text-xs text-slate-500">{a.due_at ? new Date(a.due_at).toLocaleDateString('pt-BR') : 'sem data'}</p>
            </div>
          </div>
        </div>) : <p className="rounded-xl bg-slate-50 p-3 text-sm text-slate-500">Nenhuma atividade planejada para este negócio.</p>}
      </div>
    </div>
  </section>
}
