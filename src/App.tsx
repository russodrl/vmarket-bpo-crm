...
function App() {
  ...
  if (detailPersonId) {
    return <ContactPage key={detailPersonId} person={detailPerson} organization={organizations.find((org) => org.id === detailPerson?.organization_id)} loading={loading} error={error} deals={deals} activities={activities} history={history} externalRecords={externalRecords} crmUsers={crmUsers} openDealPage={openDealPage} openOrganizationPage={openOrganizationPage} unlinkPersonOrganization={unlinkPersonOrganization} closeDetailPage={closeDetailPage} />
  }
  if (detailOrganizationId) {
    return <CompanyPage key={detailOrganizationId} organization={detailOrganization} loading={loading} error={error} deals={deals} people={people} activities={activities} history={history} externalRecords={externalRecords} crmUsers={crmUsers} openDealPage={openDealPage} openPersonPage={openPersonPage} unlinkPersonOrganization={unlinkPersonOrganization} closeDetailPage={closeDetailPage} />
  }
...
function ContactPage({ person, organization, loading, error, deals, activities, history, externalRecords, crmUsers, openDealPage, openOrganizationPage, unlinkPersonOrganization, closeDetailPage }: { person?: Person; organization?: Organization; loading: boolean; error: string; deals: Deal[]; activities: ActivityRow[]; history: HistoryRow[]; externalRecords: ExternalRecord[]; crmUsers: CrmUser[]; openDealPage: (id: string) => void; openOrganizationPage: (id: string) => void; unlinkPersonOrganization: (id: string) => Promise<void>; closeDetailPage: () => void }) {
...
function CompanyPage({ organization, loading, error, deals, people, activities, history, externalRecords, crmUsers, openDealPage, openPersonPage, unlinkPersonOrganization, closeDetailPage }: { organization?: Organization; loading: boolean; error: string; deals: Deal[]; people: Person[]; activities: ActivityRow[]; history: HistoryRow[]; externalRecords: ExternalRecord[]; crmUsers: CrmUser[]; openDealPage: (id: string) => void; openPersonPage: (id: string) => void; unlinkPersonOrganization: (id: string) => Promise<void>; closeDetailPage: () => void }) {
  if (loading) return <main className="min-h-screen bg-[#f4f5f7] p-5 text-slate-900"><LoadingBpo /></main>
